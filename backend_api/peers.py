...
def create_peer(  # noqa: C901 - function is intentionally a bit complex; refactor in follow-up
    ...
    if key_policy == "db":
        if not public:
            ...
            print(f"[DEBUG] Generated WireGuard key pair in db mode: private_key_len={len(private)}, public_key_len={len(public)}")
        else:
            ...
            print(f"[DEBUG] Client provided public key, generated server private key in db mode")
    ...
    if key_policy == "host":
        ...
        if gen:
            ...
            private, public = _generate_wg_keypair()
            print(f"[DEBUG] Host key generation failed, falling back to local generation")
    ...
    logger.info(f"[PEER_CREATED] user_id={target_user}, wg_ip={peer.wg_ip}, allowed_ips={peer.allowed_ips}")
    db.add(peer)
    try:
        db.commit()
        db.refresh(peer)
    ...
    try:
        ...
        print(f"[DEBUG] WG_APPLY_ENABLED={wg_host_module.WG_APPLY_ENABLED}, WG_HOST_SSH={wg_host_module.WG_HOST_SSH}")
        apply_peer(peer)
    except Exception as e:
